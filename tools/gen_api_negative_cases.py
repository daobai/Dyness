# -*- coding: utf-8 -*-
"""生成 API 异常/边界/特殊参数用例 CSV（tests/api/data/api_testdata_negative.csv）。

覆盖 18 个接口的：异常(deviceSn 不存在)、特殊参数(空/缺失)、边界外(越界/枚举非法)、
边界值(合法最小/最大)。预期业务码：参数错误 -> 500（接口文档 L321「500 Invalid Parameter」），
合法边界值 -> 200。运行：py -3 tools/gen_api_negative_cases.py
"""
...
OUT = Path(__file__).resolve().parent.parent / "tests" / "api" / "data" / "api_testdata_negative.csv"
...
CASES.append(neg(f"TC{_cid}", "工作模式下发-workMode非法", "/v2/SetWorkModeSetting",
                 {"deviceSn": SN, "workMode": "99", "workGroups": [_wg()]}, "边界外:workMode=99(枚举外)", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "工作模式下发-workGroups功率越界", "/v2/SetWorkModeSetting",
                 {"deviceSn": SN, "workMode": "3", "workGroups": [_wg(power="2001")]}, "边界外:workGroups.power=2001(最大2000)", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "电池参数下发-onGridDischargeDod越界", "/v2/SetBatterySetting",
                 {"deviceSn": SN, "onGridDischargeDod": "96"}, "边界外:onGridDischargeDod=96(最大95)", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "电池参数下发-onGridDischargeDod为负", "/v2/SetBatterySetting",
                 {"deviceSn": SN, "onGridDischargeDod": "-1"}, "边界外:onGridDischargeDod=-1", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "电池参数下发-batteryChargeLimit越界", "/v2/SetBatterySetting",
                 {"deviceSn": SN, "batteryChargeLimit": "101"}, "边界外:batteryChargeLimit=101(最大100)", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "负载控制下发-loadSwitch非法", "/v2/SetLoadControlSetting",
                 {"deviceSn": SN, "loadSwitch": "2"}, "边界外:loadSwitch=2(枚举0/1)", models="SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "负载控制下发-relayCloseSoc越界", "/v2/SetLoadControlSetting",
                 {"deviceSn": SN, "relayCloseSoc": "101"}, "边界外:relayCloseSoc=101(最大100)", models="SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "负载控制下发-pvPowerThreshold越界", "/v2/SetLoadControlSetting",
                 {"deviceSn": SN, "pvPowerThreshold": "65536"}, "边界外:pvPowerThreshold=65536(最大65535)", models="SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "峰值控制下发-triggerSoc越界", "/v2/SetPeakControlSetting",
                 {"deviceSn": SN, "triggerSoc": "101"}, "边界外:triggerSoc=101(最大100)", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "峰值控制下发-peakControlEnable非法", "/v2/SetPeakControlSetting",
                 {"deviceSn": SN, "peakControlEnable": "2"}, "边界外:peakControlEnable=2(枚举0/1)", models="AquaVolt,SolarCube")); _cid += 1
...
CASES.append(neg(f"TC{_cid}", "高级参数下发-isoSetting越界", "/v2/SetAdvancedSetting",
                 {"deviceSn": SN, "isoSetting": "9"}, "边界外:isoSetting=9(最小10)", models="SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "高级参数下发-gridPowerLimitValue越界", "/v2/SetAdvancedSetting",
                 {"deviceSn": SN, "gridPowerLimitGroup": {"gridPowerLimitValue": "801"}}, "边界外:gridPowerLimitValue=801(最大800)", models="AquaVolt,SolarCube")); _cid += 1
...
CASES.append(neg(f"TC{_cid}", "工作模式下发-缺workGroups", "/v2/SetWorkModeSetting",
                 {"deviceSn": SN, "workMode": "3"}, "必填缺失:缺workGroups", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "工作模式下发-缺workMode", "/v2/SetWorkModeSetting",
                 {"deviceSn": SN, "workGroups": [_wg()]}, "必填缺失:缺workMode", models="AquaVolt,SolarCube")); _cid += 1

# ===== 5. 边界值（合法，预期 200）=====
CASES.append(neg(f"TC{_cid}", "电池参数下发-onGridDischargeDod最小值", "/v2/SetBatterySetting",
                 {"deviceSn": SN, "onGridDischargeDod": "0"}, "边界值:onGridDischargeDod=0(最小)", "200", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "电池参数下发-onGridDischargeDod最大值", "/v2/SetBatterySetting",
                 {"deviceSn": SN, "onGridDischargeDod": "95"}, "边界值:onGridDischargeDod=95(最大)", "200", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "电池参数下发-batteryChargeLimit最小值", "/v2/SetBatterySetting",
                 {"deviceSn": SN, "batteryChargeLimit": "10"}, "边界值:batteryChargeLimit=10(最小)", "200", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "峰值控制下发-triggerSoc最大值", "/v2/SetPeakControlSetting",
                 {"deviceSn": SN, "triggerSoc": "100"}, "边界值:triggerSoc=100(最大)", "200", models="AquaVolt,SolarCube")); _cid += 1
...
# workMode 枚举 SolarCube 0/1/2/3/6（3 已在正常用例）
for _v, _m in [("0", "AquaVolt,SolarCube"), ("1", "AquaVolt,SolarCube"), ("2", "AquaVolt,SolarCube"), ("6", "SolarCube")]:
...
CASES.append(neg(f"TC{_cid}", "高级参数下发-extCtMeterSelect=0", "/v2/SetAdvancedSetting",
                 ...
                 "枚举值:extCtMeterSelect=0(无连接)", "200", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "高级参数下发-extCtMeterSelect=2", "/v2/SetAdvancedSetting",
                 ...
                 "枚举值:extCtMeterSelect=2(外置Meter)", "200", models="AquaVolt,SolarCube")); _cid += 1
...
CASES.append(neg(f"TC{_cid}", "电池参数下发-offGridDischargeDod越界", "/v2/SetBatterySetting",
                 {"deviceSn": SN, "offGridDischargeDod": "96"}, "边界外:offGridDischargeDod=96(最大95)")); _cid += 1
CASES.append(neg(f"TC{_cid}", "电池参数下发-batteryOffGridChargeLimit越界", "/v2/SetBatterySetting",
                 {"deviceSn": SN, "batteryOffGridChargeLimit": "101"}, "边界外:batteryOffGridChargeLimit=101(最大100)", models="SolarCube")); _cid += 1
...
CASES.append(neg(f"TC{_cid}", "负载控制下发-forceCloseTime格式错误", "/v2/SetLoadControlSetting",
                 {"deviceSn": SN, "forceCloseTime": "abc"}, "特殊参数:forceCloseTime格式错误", models="SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "负载控制下发-relayOpenSoc越界", "/v2/SetLoadControlSetting",
                 {"deviceSn": SN, "relayOpenSoc": "101"}, "边界外:relayOpenSoc=101(最大100)", models="SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "负载控制下发-forceCloseOffGridOnly非法", "/v2/SetLoadControlSetting",
                 {"deviceSn": SN, "forceCloseOffGridOnly": "2"}, "边界外:forceCloseOffGridOnly=2(枚举0/1)", models="SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "负载控制下发-alwaysCloseOnGridMode非法", "/v2/SetLoadControlSetting",
                 {"deviceSn": SN, "alwaysCloseOnGridMode": "2"}, "边界外:alwaysCloseOnGridMode=2(枚举0/1)", models="SolarCube")); _cid += 1
...
CASES.append(neg(f"TC{_cid}", "峰值控制下发-timeRange格式错误", "/v2/SetPeakControlSetting",
                 {"deviceSn": SN, "timeRange": "abc"}, "特殊参数:timeRange格式错误", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "峰值控制下发-peakControlPower越界", "/v2/SetPeakControlSetting",
                 {"deviceSn": SN, "peakControlPower": "65536"}, "边界外:peakControlPower=65536(最大65535)", models="AquaVolt,SolarCube")); _cid += 1
...
CASES.append(neg(f"TC{_cid}", "高级参数下发-backupPowerSwitch非法", "/v2/SetAdvancedSetting",
                 {"deviceSn": SN, "backupPowerSwitch": "2"}, "边界外:backupPowerSwitch=2(枚举0/1)", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "高级参数下发-pvConnectMode非法", "/v2/SetAdvancedSetting",
                 {"deviceSn": SN, "pvConnectMode": "2"}, "边界外:pvConnectMode=2(枚举0/1)", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "高级参数下发-extCtMeterSelect非法", "/v2/SetAdvancedSetting",
                 {"deviceSn": SN, "extCtMeterGroup": {"extCtMeterSelect": "3"}},
                 "边界外:extCtMeterSelect=3(枚举0/1/2)", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "高级参数下发-acCouplingEnable非法", "/v2/SetAdvancedSetting",
                 {"deviceSn": SN, "acCouplingEnable": "2"}, "边界外:acCouplingEnable=2(枚举0/1)", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "高级参数下发-groundDetectionEnable非法", "/v2/SetAdvancedSetting",
                 {"deviceSn": SN, "groundDetectionEnable": "2"}, "边界外:groundDetectionEnable=2(枚举0/1)", models="AquaVolt,SolarCube")); _cid += 1
...
CASES.append(neg(f"TC{_cid}", "高级参数下发-gridPowerLimitValue最小值", "/v2/SetAdvancedSetting",
                 {"deviceSn": SN, "gridPowerLimitGroup": {"gridPowerLimitValue": "0"}}, "边界值:gridPowerLimitValue=0(最小)", "200", models="AquaVolt,SolarCube")); _cid += 1
CASES.append(neg(f"TC{_cid}", "高级参数下发-gridPowerLimitValue最大值", "/v2/SetAdvancedSetting",
                 {"deviceSn": SN, "gridPowerLimitGroup": {"gridPowerLimitValue": "800"}}, "边界值:gridPowerLimitValue=800(最大)", "200", models="AquaVolt,SolarCube")); _cid += 1
